...
REPORT_DIR="/root/kiaan-wms/testing/reports"
FINAL_REPORT="$REPORT_DIR/COMPREHENSIVE_TEST_REPORT.md"
...
AUTH_PASS=$(grep "Passed:" $REPORT_DIR/1_auth_report.txt | awk '{print $2}' || echo "0")
AUTH_FAIL=$(grep "Failed:" $REPORT_DIR/1_auth_report.txt | awk '{print $2}' || echo "0")
API_PASS=$(grep "Passed:" $REPORT_DIR/2_api_report.txt | awk '{print $2}' || echo "0")
API_FAIL=$(grep "Failed:" $REPORT_DIR/2_api_report.txt | awk '{print $2}' || echo "0")
FE_PASS=$(grep "Passed:" $REPORT_DIR/3_frontend_report.txt | awk '{print $2}' || echo "0")
FE_FAIL=$(grep "Failed:" $REPORT_DIR/3_frontend_report.txt | awk '{print $2}' || echo "0")

TOTAL_PASS=$((AUTH_PASS + API_PASS + FE_PASS))
TOTAL_FAIL=$((AUTH_FAIL + API_FAIL + FE_FAIL))
...
|----------|-----------|--------|--------|-----------|
| Authentication & Security | $((AUTH_PASS + AUTH_FAIL)) | $AUTH_PASS | $AUTH_FAIL | $(awk "BEGIN {printf \"%.1f%%\", ($AUTH_PASS/($AUTH_PASS+$AUTH_FAIL))*100}") |
| API Endpoints | $((API_PASS + API_FAIL)) | $API_PASS | $API_FAIL | $(awk "BEGIN {printf \"%.1f%%\", ($API_PASS/($API_PASS+$API_FAIL))*100}") |
| Frontend UI | $((FE_PASS + FE_FAIL)) | $FE_PASS | $FE_FAIL | $(awk "BEGIN {printf \"%.1f%%\", ($FE_PASS/($FE_PASS+$FE_FAIL))*100}") |
| **TOTAL** | **$TOTAL_TESTS** | **$TOTAL_PASS** | **$TOTAL_FAIL** | **${PASS_RATE}%** |
...
# Add detailed test logs reference
cat >> "$FINAL_REPORT" << 'EOF'
...
echo "**Test Duration:** ~4 minutes" >> "$FINAL_REPORT"
echo "**Tester:** Claude Code (Automated Testing Framework)" >> "$FINAL_REPORT"
echo "" >> "$FINAL_REPORT"
echo "---" >> "$FINAL_REPORT"
echo "" >> "$FINAL_REPORT"
echo "🧪 **Testing Complete!** For questions or clarifications, review individual test reports." >> "$FINAL_REPORT"
